...
contract MockVoodooTreasury is Ownable {
  using SafeMath for uint256;
  ...
  uint256 public sVODOQueue; // Delays change to sVODO address
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sVODO).balanceOf(msg.sender); // Can only borrow against sVODO held
  ...
  /**
        ...
        @param _amount uint
     */
  function repayDebtWithVODO(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");
    ...
    returns (uint256 value_)
  {
    if (isReserveToken[_token]) {
      ...
      value_ = _amount.mul(10**IERC20(VODO).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SVODO) {
      // 9
