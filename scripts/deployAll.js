// @dev. This script will deploy this V1.1 of Voodoo. It will deploy the whole ecosystem except for the LP tokens and their bonds. 
// This should be enough of a test environment to learn about and test implementations with the Voodoo as of V1.1.
...
async function main() {
    ...
    // Deploy VODO
    const VODO = await ethers.getContractFactory('VoodooERC20Token');
    const ohm = await VODO.deploy();
    ...
    const Treasury = await ethers.getContractFactory('MockVoodooTreasury'); 
    const treasury = await Treasury.deploy( ohm.address, dai.address, frax.address, 0 );

    // Deploy bonding calc
    const VoodooBondingCalculator = await ethers.getContractFactory('VoodooBondingCalculator');
    const olympusBondingCalculator = await VoodooBondingCalculator.deploy( ohm.address );
    ...
    // Deploy sVODO
    const SVODO = await ethers.getContractFactory('sVoodoo');
    const sVODO = await SVODO.deploy();

    // Deploy Staking
    const Staking = await ethers.getContractFactory('VoodooStaking');
    const staking = await Staking.deploy( ohm.address, sVODO.address, epochLengthInBlocks, firstEpochNumber, firstEpochBlock );

    // Deploy staking warmpup
    const StakingWarmpup = await ethers.getContractFactory('StakingWarmup');
    const stakingWarmup = await StakingWarmpup.deploy(staking.address, sVODO.address);
    ...
    const DAIBond = await ethers.getContractFactory('MockVoodooBondDepository');
    const daiBond = await DAIBond.deploy(ohm.address, dai.address, treasury.address, MockDAO.address, zeroAddress);

    // Deploy Frax bond
    //@dev changed function call to Treasury of 'valueOf' to 'valueOfToken' in BondDepository due to change in Treausry contract
    const FraxBond = await ethers.getContractFactory('MockVoodooBondDepository');
    ...
    // Initialize sVODO and set the index
    await sVODO.initialize(staking.address);
    await sVODO.setIndex(initialIndex);

    // set distributor contract and warmup contract
    await staking.setContract('0', distributor.address);
    await staking.setContract('1', stakingWarmup.address);

    // Set treasury for VODO token
    ...
    // Approve staking and staking helper contact to spend deployer's VODO
    await ohm.approve(staking.address, largeApproval);
    await ohm.approve(stakingHelper.address, largeApproval);

    // Deposit 9,000,000 DAI to treasury, 600,000 VODO gets minted to deployer and 8,400,000 are in treasury as excesss reserves
    ...
    // Stake VODO through helper
    await stakingHelper.stake('100000000000');

    // Bond 1,000 VODO and Frax in each of their bonds
